...
import { FaEllipsisV, FaArrowUp, FaImage, FaPaperclip } from "react-icons/fa";
    ...
    const fetchConversations = async () => {
      try {
        ...
        if (conversationIdFromUrl) {
          // Check if the conversationId from URL exists in the list
          const foundConversation = transformedConversations.find((conv) => {
            const convId = conv.conversationId?.toString();
            ...
            selectedConversation.messages.length > 0 ? (
              selectedConversation.messages.map((msg) => {
                ...
                if (isSystemMessage && msg.content) {
                  if (
                    msg.content.includes("ĐÃ TẠO LỊCH HẸN") ||
                    msg.content.includes("Đã tạo lịch hẹn")
                  ) {
                    systemType = "system-create";
                  } else if (
                    msg.content.includes("ĐÃ HỦY") ||
                    msg.content.includes("ĐÃ TỪ CHỐI") ||
                    msg.content.includes("HỦY")
                  ) {
                  ...
                  >
                    ...
                    <div
                      className={`chat-message-bubble ${
                        isSystemMessage ? `system-bubble ${systemType}` : ""
                      }`}
                    >
                      ...
                      {msg.content && (
                        ...
                        >
                          {isSystemMessage ? (
                            <>
                              {/* Extract and bold the title (first line) */}
                              <div className="system-message-title">
                                {msg.content.split("\n")[0]}
                              </div>
                              {/* Show remaining content if exists */}
                              {msg.content.split("\n").slice(1).join("\n") && (
          ...
          <div className="chat-input-area">
            ...
            <div className="chat-input-row">
              ...
              <button
                ...
                disabled={
                  isSending ||
                  (!wsConnected && !selectedFile) ||
                  (!inputValue.trim() && !selectedFile)
                }
                className={`chat-send-btn ${
                  !isSending &&
                  (selectedFile || (wsConnected && inputValue.trim()))
                    ? "active"
                    : "disabled"
                }`}
              >
