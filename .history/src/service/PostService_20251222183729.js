...
export async function getPostSimilarity(id) {
  const { data } = await axios.get(`/api/post/similarity/${id}`);
  return data;
}
...
export async function getAllPosts(title, categoryName, maxDistance, page) {
  const { data } = await axios.get(
    `/api/post/search?title=${title}&categoryName=${categoryName}&maxDistance=${maxDistance}&page=${page}&size=24`
  );
  return data;
}
